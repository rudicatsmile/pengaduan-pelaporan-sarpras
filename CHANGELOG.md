...
### Fixed
- [24 Juli 2026] [Antigravity] **Bug Upload & URL Ganda Lampiran Petugas**: Memperbaiki 2 masalah sekaligus pada fitur Penyelesaian Tugas oleh Petugas (Mobile/API). (1) Mengubah batas unggahan gambar dari 2MB menjadi 5MB di backend (`TaskController`) dan mengaktifkan kompresi kamera (quality: 50) di aplikasi Flutter agar pengunggahan foto tidak gagal karena kebesaran ukuran (Error 422). (2) Memperbaiki bug penyimpanan path gambar resolusi tugas yang menyebabkan URL ter-*generate* ganda (berulang) dan mengakibatkan gambar rusak/404 saat ditampilkan di aplikasi Mobile maupun Web.
